...
Generates: `bounce` (floor), `clank` + `rattle` (rim), `thud` (backboard), `swish1-3`, `tick` (score-receipt click — pitched UP at play time via `audio.playTick` playbackRate ramp), `multhit` (×multiplier ding), `basshit` (receipt-total thump), `slowmo` (bullet-time entry whoosh — pitch-diving sine + darkening noise wash), `crowd` (3 s loopable, crossfaded head/tail), `swell` (milestone cheer). Helpers: `writeWav()`, `env()` (attack/decay), `bandNoise()`.
