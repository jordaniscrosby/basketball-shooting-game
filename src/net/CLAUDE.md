...
## verletNet.ts
`class VerletNet(scene, rimCenter)` — a Verlet-integrated cord lattice pinned to the rim, drawn as hand-inked cords via two `scene/inkRibbon.RibbonBatch`es — a dark ink stroke (`palette.ink`) with an offset paper-white core (`palette.net`).
