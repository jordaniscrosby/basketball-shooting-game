...
## tuning.ts
`export const tuning` — physics + gameplay registry, grouped by section: `world`, `ball`, `rim`, `backboard`, `floor`, `court`, `scoring`, `solver`, `input`, `slingshot`, `clickclick`, `curve`, `slowmo` (bullet time while curve-steering: time scale per star multiplier + ease rates), `trajectory` (aim-time ghost-world flight preview: enabled, horizonSec — kept short by design, boardFollowSec backboard cutoff; visual dot styling lives in `artTheme.trajectory`), `spin`, `camera`, `game`, `difficulty`, `score`, `juice`, `debug`. Also `export const derived` — live getters computed from tuning: `ballCollisionRadius`, `rimInnerRadius`, `rimCenterZ`, `backboardFaceZ`. `type Tuning = typeof tuning`.
...
- `clickclick.lateralGain`/`lateralMax` shadow `input.lateralGain`/`lateralMax` — click-click fires through `aimShot` with its own mapping (1:1 by design) instead of the swipe assist.
- Any physics-section change ⇒ re-run the shot battery (`npx vitest run src/systems/shotBattery.test.ts`).

## artTheme.ts
`export const artTheme` — visual/style registry, same discipline as tuning. Sections: `palette`, `score` (the semantic score colors — base/bonus/mult/total; the Balatro rule: the mapping never breaks across HUD, receipt cards, particles), `hud` (digit roll/stagger/overshoot/pop, idle breathe, heat escalation scales, superstar jitter, flame tongues `flameCount`/`flameEm`), `shake` (3 shake tiers + score thresholds), `cel`, `outline`, `boil`, `blobShadow`, `net`, `trail`, `trajectory` (aim-preview ink dots: color/opacity/dotRadius/everyN/endScale + the release flash releaseColor/releaseOpacity/releaseFadeSec/releaseWidth (a continuous ribbon, never dotted) — the gameplay switch + horizon/board cutoffs live in `tuning.trajectory`), `fx` (stepHz "on twos", receipt pacing, panel dim/spotlight, freezeSec, smearSec, stretch/squash, starTwinkle), `slowmoFx` (bullet-time visuals: fovScale camera pinch, warp streak/vignette opacities — the trigger/depth live in `tuning.slowmo`), `swirl` (paint-swirl cameo dials), `heatFx` (rim emissive per heat tier), `grainOpacity`. After mutating `artTheme.cel`, call `scene/toon.refreshGradientMap()` — the cel gradient texture is cached (the panel already does this). Colors + HUD motion dials reach the DOM via `ui/themeBridge.applyThemeToCss()` (CSS custom properties) — call it after mutating those outside the debug panel.
