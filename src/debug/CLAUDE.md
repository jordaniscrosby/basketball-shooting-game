...
## swipeOverlay.ts
`class SwipeOverlay(canvas)` — 2D canvas overlay for input introspection: live swipe samples, release gesture + predicted arc (main.ts passes the ghost-world prediction from `systems/trajectory.ts` — real physics, bounces included; a drag-free parabola is the fallback for callers without a predictor), steer state. Gated by `tuning.debug.swipeOverlay`/`predictedArc` — **except** `showSlingshot`/`clearSlingshot` and `showClickMeter`/`clearClickMeter`, which draw the slingshot pull-back and click-click charge feedback and are real always-on gameplay UI. Both power meters share `drawArcadeMeter`: a red→orange→yellow→green→…→red gradient track built around each meter's sweet spot (`meterColorAt` normalizes each side independently, so both ends hit full red even off-center), ink border, sweet-spot notch, white pointer. Colors here are hardcoded RGBA (dev chrome, deliberately outside artTheme).
